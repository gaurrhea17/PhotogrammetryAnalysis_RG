...
#             self.seed_feature_locations[i] = f
    ...
    def reprojection_errors(self, camera_rotations, camera_translations, feature_locations,
        ...
        errors = pd.DataFrame(columns=['feature_id', 'image_id', 'error_x', 'error_y'])
        reprojected = pd.DataFrame(columns=['feature_id', 'image_id', 'x', 'y'])
        for i in range(self.nimages):
            indices = np.where(np.any(self.image_feature_locations[i] != 0, axis=1))[0] #

            get_reproject = cv2.projectPoints(feature_locations[indices], camera_rotations[i],
                                            camera_translations[i], self.camera_matrix,
                                            self.distortion)[0].reshape((indices.size, 2))

            reprojected = pd.concat([reprojected, pd.DataFrame({'feature_id': [self.index_feature[ii] for ii in indices],
                                                    'image_id': self.index_image[i],
                                                    'x': get_reproject[:,0],
                                                    'y': get_reproject[:,1]})])

            errors = pd.concat([errors, pd.DataFrame({'feature_id': [self.index_feature[ii] for ii in indices],
                                                    'image_id': self.index_image[i],
                                                    'error_x': get_reproject[:,0] - self.image_feature_locations[i][indices,0],
                                                    'error_y': get_reproject[:,1] - self.image_feature_locations[i][indices,1]})])
        errors['RE'] = linalg.norm(errors[['error_x', 'error_y']], axis=1)

        return errors, reprojected
    ...
    def fit_errors(self, params, fit_cam=False, max_error=None):
        ...
        errors, reprojected = self.reprojection_errors(camera_rotations, camera_translations, feature_locations, camera_matrix, distortion)
        if max_error is None:
            return np.ravel(errors[['error_x', 'error_y']].values)
        return np.minimum(errors['RE'], max_error)
    ...
    def bundle_adjustment(self, camera_rotations, camera_translations, xtol=1e-9, method='trf', use_sparsity = True,
        ...
        initial_errors = self.fit_errors(x0, fit_cam, max_error)
...
def kabsch_errors(base_feature_locations, reco_feature_locations):
    ...
    # convert reco_transformed and errors to pandas df
    reco_transformed_df = pd.DataFrame(reco_transformed, columns=['x', 'y', 'z'])
    reco_transformed_df['feature_id'] = reco_feature_locations.keys()
    errors = pd.DataFrame(errors, columns=['x', 'y', 'z'])
    errors['feature_id'] = reco_feature_locations.keys()
    errors['residual'] = np.sqrt(errors['x'] ** 2 + errors['y'] ** 2 + errors['z'] ** 2)
    return errors, reco_transformed_df, scale, R, translation, base_location_mean
...
def read_3d_feature_locations(filename, delimiter="\t", poly=False):
    with open(filename, mode='r') as file:
        reader = csv.reader(file, delimiter=delimiter)
        if poly == True:
            feature_locations = {r[0]: np.array([r[1], r[2], r[3], r[4]]).astype(float) for r in reader} # reads in PMT ID,
        # x, y, z coordinates
        else:
